...
void Conf_LCD1602(void)
{
    ...
    for(Delay=300; Delay>0; Delay--);//20 ms aprox a 16MHz//tenia delay=30000
    //manda 0x3 en la parte alta de la LCD
    D4_OUT|=D4;
    D5_OUT|=D5;
    LCD1602_E(); //manda un pulso en el pin E (Enable)
    for(Delay=1000; Delay>0; Delay--);//7 ms aprox a 16MHz//Delay=10000

    LCD1602_E(); //manda un pulso en el pin E (Enable)
    for(Delay=500; Delay>0; Delay--);//100 us aprox a 16MHz

    LCD1602_E(); //manda un pulso en el pin E (Enable)
    for(Delay=1000; Delay>0; Delay--);//7 ms aprox a 16MHz
